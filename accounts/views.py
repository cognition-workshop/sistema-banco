    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if registration_form.is_valid() and address_form.is_valid():
                ...
                logger.info(
                    f"Novo usuário registrado: {user.email}, Conta: {user.account.account_no}"
                )
                ...
                logger.warning(
                    f"Falha no registro: {registration_form.errors}, {address_form.errors}"
                )
        except Exception as e:
            logger.error(f"Erro durante registro: {str(e)}", exc_info=True)
            messages.error(
                self.request,
