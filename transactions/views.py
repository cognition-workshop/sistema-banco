...
class DepositMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        demo_user = User.objects.filter(email="demo@example.com").first()
        account = (
            demo_user.account if demo_user and hasattr(demo_user, "account") else None
        )
        if not account:
            logger.error("Deposit failed: No account found for demo user")
            return super().form_valid(form)

        logger.info(f"Deposit initiated: Account {account.account_no}, Amount {amount}")
        ...
        logger.info(
            ...
            f"Amount {amount}, Old balance {old_balance}, "
...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if demo_user and hasattr(demo_user, "account"):
            logger.info(
                f"Withdrawal initiated: Account {demo_user.account.account_no}, Amount {amount}"
            )
            ...
            logger.info(
                ...
                f"Amount {amount}, Old balance {old_balance}, "
                f"New balance {demo_user.account.balance}"
            )
        else:
            logger.error("Withdrawal failed: No account found for demo user")
